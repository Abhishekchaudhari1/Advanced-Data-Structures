...
public:
    ...
    void searchstudent(sms *root, string ss, string mothername); // Updated
    void modifystudent(sms *root, string ss, string mothername); // Updated
    int height(sms *root);
    int countnode(sms *root);
    void displayroot(sms *root);
    void mirror(sms *root);
    void topper(sms *root);
    void lowestScore(sms *root); // Added
...
void sms::display(sms *root)   //Non recursive in-order traversal
{
    int i = 0, top = -1;
    temp = root;
    if (temp == NULL)
    ...
    for (int j = 0; j < i; j++)
    {
        cout << arr[j]->name << "\t" << arr[j]->email << "\t" << arr[j]->seatno << "\t" << arr[j]->cet << "\t" << arr[j]->mothername << endl;
    }
}
